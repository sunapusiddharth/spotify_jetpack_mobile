...
import { browsePlayListCollectionByGenre } from "../../../api";
import { ArtistCard } from "../../../components/artist/ArtistCard";
import { AppContext, AppContextType } from "../../_app";
interface PageProps {

}
const PlcGenresPageComp: React.FC<PageProps> = ({
}: PageProps) => {
  ...
  const { isSuccess,isLoading,isError,data,error,hasNextPage,fetchNextPage,isFetchingNextPage} = useInfiniteQuery(
    ["plc_genres", genre],
    async ({ pageParam = 1 }) => await browsePlayListCollectionByGenre(genre, pageParam), {
        ...
        {isSuccess && data?.pages && <>
          <Grid container spacing={{ xs: 2, md: 4 }} columns={{ xs: 2, md: 5 }} justifyContent='center'>
            {data.pages?.map((page, i) => (
              <>
                {page.results?.map((artist, j) => <Link href={`/playlist_collection/${artist.id}`} key={'artist-' + artist.id}>
...
export const PlcGenresPage = React.memo(
  PlcGenresPageComp
  // propsAreEqual
);
export default PlcGenresPage;
